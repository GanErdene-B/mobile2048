...
class ApiService {
  ...
  static final String _envBase = const String.fromEnvironment('API_BASE_URL', defaultValue: '');
  static final String _apiKey = const String.fromEnvironment('API_KEY', defaultValue: 'uz5y7iu9x1mnf4atz2g8jr4he');
    ...
    for (final candidate in _candidates) {
      ...
      for (int attempt = 0; attempt < _maxRetries; attempt++) {
        try {
          ...
          final resp = await http.get(uri, headers: _defaultHeaders()).timeout(const Duration(seconds: 2));
          ...
          final resp2 = await http.get(uri2, headers: _defaultHeaders()).timeout(const Duration(seconds: 2));
  ...
  // POST /api/save
  static Future<Map<String, dynamic>> saveGame(Map<String, dynamic> payload) async {
  ...
  static Future<List<Map<String, dynamic>>> getLeaderboard() async {
    ...
    final resp = await http.get(uri, headers: _defaultHeaders()).timeout(const Duration(seconds: 6));
    if (resp.statusCode >= 200 && resp.statusCode < 300) {
